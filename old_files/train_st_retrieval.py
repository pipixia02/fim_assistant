...
train_path = "../dataset/retrieval_data/new2/train.json"
...
def train_epoch(model: nn.Module,
    ...
    validation_interval = steps_per_epoch // 10  # 每1/4 epoch验证一次
    best_loss = float('inf')
    ...
    with tqdm(dataloader, desc="Training") as pbar:
        for step, batch in enumerate(pbar):
            ...
            if step > 0 and step % validation_interval == 0:
                ...
                # 保存最佳模型
                if dev_loss < best_loss and dev_loss < 0.6:
                    best_loss = dev_loss
...
def save_model(model, output_dir, loss):
    """保存模型和领域适应层"""
    # 保存模型
    model_path = os.path.join(output_dir, f"best_model_{loss:.3f}")
...
def main():
    ...
    # 初始化模型
    model = STRetriever(model_path=model_path, use_domain_adaptation=True)
    model.to(device)
